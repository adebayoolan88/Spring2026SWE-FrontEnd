...
const CATEGORIES = ["All", "Guitars", "Keyboards", "Drums", "Audio", "Accessories"];
  ...
  const filteredItems = useMemo(() => {
    ...
    return ITEMS.filter((item) => {
      ...
      const matchesCategory =
        activeCategory === "All" ||
        item.category.toLowerCase() === activeCategory.toLowerCase().slice(0, -1) ||
  ...
  return (
    <div className="min-h-screen bg-[#f5f7fb] text-slate-900">
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <section className="grid gap-6 rounded-[36px] bg-gradient-to-br from-slate-950 via-slate-900 to-slate-800 px-6 py-8 text-white shadow-[0_25px_60px_rgba(15,23,42,0.22)] lg:grid-cols-[1.2fr_0.8fr] lg:px-10 lg:py-10">
          <div className="flex flex-col justify-center">
            ...
            <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-4">
              ...
              <div className="rounded-3xl border border-white/10 bg-white/5 p-4">
                <p className="text-sm text-slate-400">Verified Sellers</p>
                <p className="mt-2 text-2xl font-semibold">3.1k</p>
              </div>
              <div className="rounded-3xl border border-white/10 bg-white/5 p-4">
              ...
              <div className="rounded-3xl border border-white/10 bg-white/5 p-4">
                ...
                <p className="mt-2 text-2xl font-semibold">1.9k</p>
              </div>
            </div>
          </div>

          <div className="hidden items-center justify-center lg:flex">
            <div className="w-full max-w-md rounded-[32px] border border-white/10 bg-white/8 p-5 shadow-2xl backdrop-blur-xl">
        ...
        <section className="mt-8 rounded-[32px] border border-slate-200/70 bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,0.06)]">
